...
namespace NetCore.Coupon.API.Controllers
{
    ...
    public class ProductTopicController : Controller
    {
        ...
        {
            return await memoryCache.GetOrCreate($"temai_product{t}_{pageno}_{pagesize}_{sort}", (entry) =>
            {
        ...
        {
            return await memoryCache.GetOrCreate($"saletop_product{t}_{pageno}_{pagesize}_{sort}", (entry) =>
            {
        ...
        [Route("Channel")]
        public async Task<ProductListResponse> Channel(int t = 0, int pageno = 0, int cat = 0, int pagesize = 50, int sort = 0, string ak = "")
        {
            return await memoryCache.GetOrCreate($"channel_product{t}_{pageno}_{cat}_{sort}", (entry) =>
            {
                entry.AbsoluteExpirationRelativeToNow = TimeSpan.FromSeconds(30);
                return productTopicService.Channel(new ProductTopicRequest() { Type = t, Cat = cat, PageNo = pageno, Sort = sort });
            });
        }
        [Route("All")]
        public async Task<ProductListResponse> All(int t = 0, int pageno = 1, int pagesize = 50, int sort = 0, string ak = "")
        {
            return await memoryCache.GetOrCreate($"all_product{t}_{pageno}_{pagesize}_{sort}", (entry) =>
            {
                entry.AbsoluteExpirationRelativeToNow = TimeSpan.FromSeconds(30);
